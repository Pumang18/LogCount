    ...
    try:
        ...
        with gzip.open(file_path, 'rt',encoding='utf-8', errors='ignore') as file:
            for line in file:
                ...
                if "{Customer Error}" in line:
                    split_string_from = "{Customer Error}"
                else:
                    continue
...
def main(file_name):
    ...
    # output_file = "{0}/{1}_cust_output.csv".format(output_file_path,file_name)
    output_file = os.path.join(output_file_path, f"{os.path.splitext(os.path.basename(file_name))[0]}_cust_output.csv")
